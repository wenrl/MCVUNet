...
class DownDoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, first=False, use_att=False, stride=1):
        ...
        self.DWconv1 = nn.Conv2d(out_channels * stride, out_channels, kernel_size=3, padding=1, groups=out_channels)
        # self.bn0 = nn.BatchNorm2d(out_channels)
        self.PWconv1 = nn.Conv2d(out_channels, out_channels, kernel_size=1, stride=1, padding=0)
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.fusion = nn.Sequential(nn.Conv2d(out_channels, out_channels, kernel_size=1, stride=1, padding=0),
                                    nn.BatchNorm2d(out_channels),
        ...
        if self.att == True:
            ...
            # self.MSA = MHSAblock(inplanes=out_channels, planes=out_channels)###MHSA
            self.MSA = MHSAblock_pro(inplanes=out_channels * stride, planes=out_channels) ###MHSA2 best
            # self.MSA = MHSAblockv2(inplanes=out_channels * stride, planes=out_channels)   ###MHSA2 v2
            # self.lite_se = LiteSqueezeExcitation(out_channels * stride)# no
            self.lite_se = ChannelAttention(out_channels * stride, reduction=8, kernels=(7,3,5))#0,1
    ...
    def forward(self, x):
        # B, C, H, W = x
        residual = x
        # print(self.step(x).shape, self.ds(x).shape)
        if self.stride == 2:
            x = torch.concat([self.step(x), self.ds(x)], dim=1)
